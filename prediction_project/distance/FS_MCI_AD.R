...
# between 27-62, get the index has value 1 in the .AD column in fixed.assign
assign_AD <- as.data.frame(rownames(fixed.assign)[27:62][which(fixed.assign[27:62, ".AD"] == 1)])
colnames(assign_AD) <- "caseid"
# convert to numeric
assign_AD$caseid <- as.numeric(assign_AD$caseid)
...
new_design <- bins.table$group
# if the rownames of bins.table$group is in the assign_AD$caseid, then assign it to AD
for (i in 1:nrow(bins.table)){
  if(rownames(bins.table)[i] %in% assign_AD$caseid){
    new_design[i] <- "AD"
  }
}

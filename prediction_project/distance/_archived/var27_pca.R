...
table <- as.data.frame(read_excel(file_path, sheet = "var27", skip = 1))
...
SimFreq.table <- table[30:36]
m.color.design[30:36] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))

sc.table <- table[37:38]
...
# get some color
col4Levels <- data4PCCAR::coloringLevels(
  rownames(res.PCA$ExPosition.Data$fj), m.color.design[13:end_col])
col4Labels <- col4Levels$color4Levels
...
# rename fi
rownames(res.PCA$ExPosition.Data$fi) <- bins.table$caseid
...
plot.loading(bins.table[13:end_col], col=m.color.design[13:end_col], res.PCA$ExPosition.Data$fi, res.PCA$ExPosition.Data$eigs, res.PCA$ExPosition.Data$t, d=1)

plot.cfs(res.PCA$ExPosition.Data$fj, res.PCA$ExPosition.Data$eigs, res.PCA$ExPosition.Data$t, d=1, col=col4Labels, method="MCA", colrow="row")

plot.cb(res.PCA$ExPosition.Data$cj, res.PCA$ExPosition.Data$fj, col = col4Labels, boot.ratios=res.PCA.inf$Inference.Data$fj.boots$tests$boot.ratios, signifOnly = TRUE, fig = 2, horizontal = FALSE, colrow = "col", fontsize = 2)

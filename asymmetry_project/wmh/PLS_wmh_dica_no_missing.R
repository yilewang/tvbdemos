...
table <- as.data.frame(read_excel(file_path, sheet = "var27_spike_burst_no_missing", skip = 1))
...
# Each block
ignition.table <- table[13:20]
m.color.design[13:20] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))

tvb_para.table <- table[21:26]
m.color.design[21:26] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))

MC.table <- table[27:29]
m.color.design[27:29] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))

SimFreq.table <- table[30:46]
m.color.design[30:46] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))

sc.table <- table[47:48]
m.color.design[47:48] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))

wdc.table <- table[49]
m.color.design[49] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))

wmh.table <- table[50:52]
m.color.design[50:52] <- prettyGraphsColorSelection(starting.color = sample(1:170, 1))
...
res.DICA <- tepDICA(bins.table[13:end_col], make_data_nominal = TRUE, symmetric = TRUE,
...
nIter = 1000
res.DICA.inf <- tepDICA.inference.battery(bins.table[13:end_col], make_data_nominal = TRUE, symmetric = TRUE,
                                          DESIGN = table$group,
                                          group.masses = g.masses,
                                          test.iters = nIter,
                                          #weight = rep(1, nrow(XYmat)), # -- if equal weights for all columns,
...
plot.permutation(res.DICA.inf$Inference.Data$components$eigs.perm, res.DICA$TExPosition.Data$eigs,para1 = 0.1)
...
plot.loading(bins.table[13:end_col], col=m.color.design[13:end_col], res.DICA$TExPosition.Data$fii, res.DICA$TExPosition.Data$eigs, res.DICA$TExPosition.Data$t, d=1)
